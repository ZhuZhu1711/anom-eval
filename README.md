...
对照实现来自 `Find_ClusterType_in_binarymap_Type15`（二值坏点图、4 像素打包、OTP type 1–15）。实验组只改**水平方向查点**，用来补上「能被 DPC 放过的单点 / 相邻双点，一旦和相邻簇拼成更复杂模式」时的漏检。
...
- 孤立水平单坏点：DPC 可消，**不应该**记簇。
- 孤立水平相邻双坏点：DPC 可消，**故意 skip**。
- 上面两类一旦和**相邻簇**在组边界上形成同通道、距离 2 的连接，拼成 DPC 去不掉的更复杂模式，**必须**记下来。
- 两个隔组相对的单点（例如 `..X.X...`）几何上等于组内的 `0b1010`，但按产品定义仍是两个可 DPC 单点，实验组**不**把它们配成簇。原算法因为 `map_img[loca+4]==255` 会误伤这一类，报告里会标出来。
...
实验组用「和邻组是否同通道相连 + 邻组是不是簇（已记录的组内簇，或残留的 ≥2 坏点组）」一次扫完，并把**完整 mask** 写入 `type_map`（相邻双点不再被压成单 bit）。
